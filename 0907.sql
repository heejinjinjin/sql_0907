...
-- 가격이 18,000원인 도서를 주문한 고객의 이름과 도서의 이름을 구하시오
select name, bookname from customer c, orders o, book b
where c.custid = o.custid and o.bookid = b.bookid and price = 18000;

-- 외부 조인 (Outer Join)
-- 동등조인을 하게 되면 값이 동등하지 않은 행은 무시되지만
-- 외부조인을 하게 되면 기준이 되는 테이블의 값이 동등하지 않은 데이터 행도 포함해서 출력해준다.
...
where book.publisher='좋은책';
-- 도서를 구매한 사람의 고객번호
select distinct custid from orders;
-- 좋은책에서 출판한 도서를 구매한 고객의 이름을 출력하시오
select name from customer
where custid in (select distinct custid from orders
...
where publisher='좋은책' and b.bookid = o.bookid and c.custid = o.custid;